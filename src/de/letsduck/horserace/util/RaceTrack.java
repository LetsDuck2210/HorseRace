	...
	public void setup() {
		...
		if(Main.isEnabled) {
			...
			countdownTask = Bukkit.getScheduler().scheduleSyncRepeatingTask(Main.getPlugin(), () -> {
				if(countdown.get() > 0) {
					if(countdown.get() == 3)
						Bukkit.getOnlinePlayers().forEach((player) -> player.showTitle(title("§a3", "", 5, 20, 5)));
					else if(countdown.get() == 2)
						Bukkit.getOnlinePlayers().forEach((player) -> player.showTitle(title("§62", "", 5, 20, 5)));
					else if(countdown.get() == 1)
						Bukkit.getOnlinePlayers().forEach((player) -> player.showTitle(title("§c1", "", 5, 20, 5)));
				...
				getCompeting().forEach((horse) -> {
					horse.getPassengers().forEach((entity) -> {
						if(entity instanceof Player player)
							player.showTitle(title("§aRunde 1", "�20.0s", 5, 20, 5));
					});
